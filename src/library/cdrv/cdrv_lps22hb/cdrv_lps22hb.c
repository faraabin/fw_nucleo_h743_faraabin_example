...
static uint8_t I2cRead(sCdrvLps22hbObj * const me, uint8_t regAddress, uint8_t *buffer, uint16_t size);
static uint8_t I2cWriteByte(sCdrvLps22hbObj * const me, uint8_t regAddress, uint8_t value);

/* Variables -----------------------------------------------------------------*/

/*
===============================================================================
                        ##### cdrv_lps22hb.c Exported Functions #####
===============================================================================*/
/**
 * @brief 
 * 
 ...
 * @return uint8_t 
 */
uint8_t fCdrvLps22hb_Init(sCdrvLps22hbObj * const me, uint8_t ad0State) {
  ...
  if(meI2c_Init_(me->I2c) != 0) {
	...
	fCdrvLps22hb_SoftwareReset(me);
	fCdrvLps22hb_EnableContinuousMode(me);
	if(fCdrvLps22hb_ReadId(me) != LPS22HB_WHO_AM_I_VAL) {
		return 1;
	}
...
void fCdrvLps22hb_SoftwareReset(sCdrvLps22hbObj * const me) {
	uint8_t swReset = 0x04;
	
	I2cWriteByte(me, LPS22HB_CTRL_REG2, swReset);
}
...
uint8_t fCdrvLps22hb_EnableContinuousMode(sCdrvLps22hbObj * const me) {
	uint8_t ctrlRegBuff = 0;
	I2cRead(me, LPS22HB_CTRL_REG1, &ctrlRegBuff, 1);
	ctrlRegBuff |= 0x50;
	I2cWriteByte(me, LPS22HB_CTRL_REG1, ctrlRegBuff);
	I2cRead(me, LPS22HB_CTRL_REG1, &ctrlRegBuff, 1);
	if((ctrlRegBuff & 0x50) == 0) {
		return 1;
	}
...
uint8_t fCdrvLps22hb_ReadId(sCdrvLps22hbObj * const me) {
  ...
  if(I2cRead(me, LPS22HB_WHO_AM_I_REG, &rxByte, 1) != 0) {
    return 0;
  }
...
uint8_t fCdrvLps22hb_ReadData(sCdrvLps22hbObj * const me) {
	...
	Tyu_4Byte pressure;
	Tyu_2Byte temperature;
	
	if(I2cRead(me, LPS22HB_PRESS_OUT_XL_REG, buffer, sizeof(buffer)) != 0) {
		return 1;
	}
	
	pressure.byte[0] = buffer[0];
	pressure.byte[1] = buffer[1];
	pressure.byte[2] = buffer[2];
	pressure.byte[3] = 0x00;
	
	pressure.i32 *= 256;
	
	temperature.byte[0] = buffer[3];
	temperature.byte[1] = buffer[4];
	
	me->Pressure = (float)(pressure.i32) / 1048576.0f;
	me->Temperature = (float)(temperature.i16) / 100.0f;
...
/*
===============================================================================
                    ##### cdrv_lps22hb.c Private Functions #####
===============================================================================*/
/**
 * @brief 
 * 

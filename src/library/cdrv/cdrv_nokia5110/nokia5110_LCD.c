...
void LCD_send(uint8_t val){
  ...
  for(i = 0; i < 8; i++){
		meGpio_Write_(bsp.Lcd.Nokia5110.DataIn, !!(val & (1 << (7 - i))));
		meGpio_Write_(bsp.Lcd.Nokia5110.Clock, 1);
		meGpio_Write_(bsp.Lcd.Nokia5110.Clock, 0);
...
void LCD_write(uint8_t data, uint8_t mode){
  if(mode == LCD_COMMAND){
		meGpio_Reset_(bsp.Lcd.Nokia5110.DataCommandPin);
		meGpio_Reset_(bsp.Lcd.Nokia5110.CsPin);
...
//    HAL_GPIO_WritePin(lcd_gpio.CEPORT, lcd_gpio.CEPIN, GPIO_PIN_RESET);
    LCD_send(data);
		meGpio_Set_(bsp.Lcd.Nokia5110.CsPin);
//    HAL_GPIO_WritePin(lcd_gpio.CEPORT, lcd_gpio.CEPIN, GPIO_PIN_SET);
  }
  else{
		meGpio_Set_(bsp.Lcd.Nokia5110.DataCommandPin);
		meGpio_Reset_(bsp.Lcd.Nokia5110.CsPin);
 ...
 */
void LCD_init(){
	meGpio_Reset_(bsp.Lcd.Nokia5110.ResetPin);
	meGpio_Set_(bsp.Lcd.Nokia5110.ResetPin);
...
/*
 * draw symbol and gestures
 */
void antenna1(void) { // signal weak
	for (int h = 0; h < antenna_rows; h++) {
		LCD_goXY(0, 0);
		for (int w = 0; w < antenna_cols; w++) {
			LCD_write(antenna[(h * 19) + w], 1);
		}
	}
}

void antenna2(void) { //signal normal
	for (int h = 0; h < antenna_rows; h++) {
		LCD_goXY(0, 0);
		for (int w = 0; w < antenna_cols; w++) {
			LCD_write(antenna[(h * 19) + w], 1);
...
void antenna3(void) { //signal full
	for (int h = 0; h < antenna_rows; h++) {
		LCD_goXY(0, 0);
		for (int w = 0; w < antenna_cols; w++) {
			LCD_write(antenna[(h * 19) + w], 1);
...
void charge1() { //low charge
	for (int h = 0; h < LowCharge_rows; h++) {
		LCD_goXY(12, 0);
		for (int w = 0; w < LowCharge_cols; w++) {
			LCD_write(LowCharge[(h * 19) + w], 1);
		}
	}
}

void charge2() { //medium charge
	for (int h = 0; h < MediumCharge_rows; h++) {
		LCD_goXY(12, 0);
		for (int w = 0; w < MediumCharge_cols; w++) {
			LCD_write(MediumCharge[(h * 19) + w], 1);
		}
	}
}

void charge3() { //full charge
	for (int h = 0; h < fullcharge_rows; h++) {
		LCD_goXY(12, 0);
		for (int w = 0; w < fullcharge_cols; w++) {
			LCD_write(fullcharge[(h * 19) + w], 1);
		}
	}
}

void valveON() { //open valve
	for (int h = 0; h < ValveOn_rows; h++) {
		LCD_goXY(60, 0);
		for (int w = 0; w < ValveOn_cols; w++) {
			LCD_write(ValveOn[(h * 19) + w], 1);
		}
	}
}

void valveOFF() { //close valve
	for (int h = 0; h < ValveOff_rows; h++) {
		LCD_goXY(60, 0);
		for (int w = 0; w < ValveOff_cols; w++) {
			LCD_write(ValveOff[(h * 19) + w], 1);
		}
	}
}
void flow_ForwardDIR() { //flow forward direction
	for (int h = 0; h < Forward_rows; h++) {
		LCD_goXY(30, 0);
		for (int w = 0; w < Forward_cols; w++) {
			LCD_write(Forward[(h * 19) + w], 1);
		}
	}
}

void flow_ReverseDIR() { //flow reverse direction
	for (int h = 0; h < Reverse_rows; h++) {
		LCD_goXY(30, 0);
		for (int w = 0; w < Reverse_cols; w++) {
			LCD_write(Reverse[(h * 19) + w], 1);
		}
	}
}

void Unit(void) { // unit =
	for (int h = 0; h < unit_rows; h++) {
		LCD_goXY(72, 0);
		for (int w = 0; w < unit_cols; w++) {
			LCD_write(unit[(h * 8) + w], 1);
		}
	}
}
